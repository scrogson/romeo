defmodule Romeo.StanzaTest do
  ...
  test "start_stream with default xmlns" do
    assert Stanza.start_stream("im.wonderland.lit") |> Stanza.to_xml() ==
             "<stream:stream to='im.wonderland.lit' version='1.0' xml:lang='en' xmlns='jabber:client' xmlns:stream='#{
               ns_xmpp()
             }'>"
  end
  ...
  test "subscribe" do
    assert Stanza.subscribe("pubsub.wonderland.lit", "posts", "alice@wonderland.lit")
           |> Stanza.to_xml() =~
  ...
  test "auth anonymous" do
    assert Stanza.auth("ANONYMOUS") |> Stanza.to_xml() ==
             "<auth xmlns='#{ns_sasl()}' mechanism='ANONYMOUS'/>"
  end
